...
while (my $s = <$file>)
{
	...
	{				
		...
		push (@body, "  get_target_property(dir \${name} ARCHIVE_OUTPUT_DIRECTORY)\n");
		push (@body, "  install(FILES \${dir}/Debug/\${name}.pdb CONFIGURATIONS Debug DESTINATION lib\${LLVM_LIBDIR_SUFFIX})\n");
	}
